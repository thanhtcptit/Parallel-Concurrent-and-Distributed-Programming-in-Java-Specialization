...
public final class StudentAnalytics {
    ...
    public double averageAgeOfEnrolledStudentsParallelStream(
            final Student[] studentArray) {
        return Stream.of(studentArray).parallel().filter(Student::checkIsCurrent).mapToDouble(
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
            final Student[] studentArray) {
        ArrayList<String> firstNameOccuranceSorted = new ArrayList<>();
        Stream.of(studentArray).parallel().filter(s -> !s.checkIsCurrent()).collect(
                Collectors.groupingBy(Student::getFirstName, Collectors.summingInt(a -> 1))).entrySet().stream().parallel()
                .sorted(Map.Entry.comparingByValue(Comparator.reverseOrder())).forEachOrdered(x -> firstNameOccuranceSorted.add(x.getKey()));
        return firstNameOccuranceSorted.get(0);
    }
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
            final Student[] studentArray) {
        return (int) Stream.of(studentArray).parallel().filter(s -> (!s.checkIsCurrent() && s.getGrade() < 65 && s.getAge() > 20)).count();
    }
}
