...
public class MatrixMult {
    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c,
        ...
        final int start = rank * rowChunk;
        int end = (rank + 1) * rowChunk;
        ...
        if (rank == 0) {
            ...
            for (int i = 1; i < size; i++) {
                final int rankStart = i * rowChunk;
                int rankEnd = (i + 1) * rowChunk;
                if (rankEnd > nRows) rankEnd = nRows;

                final int rowOffset = rankStart * c.getNCols();
                final int n = (rankEnd - rankStart) * c.getNCols();
                requests[i - 1] = mpi.MPI_Irecv(c.getValues(), rowOffset, n, i, i, mpi.MPI_COMM_WORLD);
            }
            mpi.MPI_Waitall(requests);
        } else {
            mpi.MPI_Send(c.getValues(), start * c.getNCols(), (end - start) * c.getNCols(), 0, rank,
                    mpi.MPI_COMM_WORLD);
        }
    }
}
